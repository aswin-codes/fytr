...
const ThemeSettingsScreen = () => {
  ...
  const handleThemeChange = (selectedTheme: 'light' | 'dark' | 'system') => {
    ...
    if (selectedTheme === 'system') {
      // For system theme, you might want to add logic to detect system theme
      // For now, we'll just toggle based on current mode
      toggleTheme();
    } else if (selectedTheme !== mode) {
      toggleTheme();
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-background-light dark:bg-background-dark">
      ...
      >
        ...
        <View className={`bg-primary/15 dark:bg-primary/10 border border-primary/20 rounded-2xl p-4 mb-8`}>
          <Text 
            className={`text-xs ${isDarkMode ? 'text-primary' : 'text-primary-glow'} leading-relaxed`}
            style={{ fontFamily: fontFamily.regular }}
          >
